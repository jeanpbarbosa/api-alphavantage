...
$url = "https://www.alphavantage.co/query?function=DIGITAL_CURRENCY_DAILY&datatype=json&symbol=" . $symbol . "&market=USD&apikey=...";
...
$array = $array["Time Series (Digital Currency Daily)"];
...
foreach ($array as $key => $value) {
	...
	if ($Banco == 0) {
		...
		$Banco->bindParam(':date', $key);
		$Banco->bindParam(':symbol', $symbol);
		$Banco->bindParam(':value', $array[$key]["4a. close (USD)"]);
		$Banco->execute();
	} else {
		// UPDATE
